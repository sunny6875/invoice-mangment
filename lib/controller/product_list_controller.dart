import 'package:invoice_management_system_flutter_ui/imports.dart';

class ProductListController {
  final list = [
    EmployeeModel(
      title: 'Motherboard',
      subTitle: '23',
    //   trailing:
    //       'https://images.unsplash.com/photo-1518770660439-4636190af475?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80',
    ),
    EmployeeModel(
      title: 'CPU',
      subTitle: '432',
    //   trailing:
    //       'https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1074&q=80',
    ),
    EmployeeModel(
      title: 'Graphic Card',
      subTitle: '12',
    //   trailing:
    //       'https://images.unsplash.com/photo-1591488320449-011701bb6704?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80',
     ),
    EmployeeModel(
      title: 'Hard Disk',
      subTitle: '54',
    //   trailing:
    //       'https://images.unsplash.com/photo-1531492746076-161ca9bcad58?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1074&q=80',
    ),
    EmployeeModel(
      title: 'Ram',
      subTitle: '33',
    //   trailing:
    //       'https://images.unsplash.com/photo-1562976540-1502c2145186?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1631&q=80',
    ),
  ];
}
